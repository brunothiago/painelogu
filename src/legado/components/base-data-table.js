...
function chooseFilterType(rows, column) {
  if (SELECT_COLUMNS.has(column)) return "select";
...
function getColumnClassName(column) {
  if (column === "operacao" || column === "proposta" || column === "instrumento") return "col-tci";
  if (column === "tipo_operacao" || column === "ativo") return "col-secretaria";
  if (column.startsWith("dt_") || column === "previsao_retomada") return "col-date";
  if (column.startsWith("vlr_")) return "col-money";
  if (column === "modalidade") return "col-medium";
  if (
    column === "situacao_obra" ||
    column === "situacao_contrato" ||
    column === "modalidade_cc" ||
    column === "recebedor" ||
    column === "municipio" ||
    column === "objeto" ||
    column === "motivo_paralisacao" ||
    column === "plano_acao"
  ) return "col-long";
  return "col-default";
}
...
function getColumnWidth(column) {
  if (column === "ativo") return "78px";
  if (column === "tipo_operacao") return "118px";
  if (column === "operacao" || column === "proposta" || column === "instrumento") return "104px";
  if (column === "uf") return "48px";
  if (column === "regiao") return "104px";
  if (column.startsWith("dt_") || column === "previsao_retomada") return "104px";
  if (column === "pct_fisico") return "96px";
  if (column === "mes_ultimo_desbloqueio") return "104px";
  if (column.startsWith("vlr_")) return "132px";
  if (column === "modalidade") return "150px";
  if (column === "modalidade_cc") return "176px";
  if (column === "situacao_obra" || column === "situacao_contrato") return "176px";
  if (column === "recebedor" || column === "municipio") return "200px";
  if (column === "objeto" || column === "motivo_paralisacao" || column === "plano_acao") return "240px";
  return "132px";
}
...
export function renderBaseDataTable({
  ...
  const dtColumns = columnMeta.map((column) => ({
    ...
    render(data, type, row) {
      ...
      // Só usar timestamp na ordenação; busca/filtro da coluna precisa bater com o texto exibido.
