# Diferença da base PC 32 - 2026-06-08

- Snapshot atual: `2026-06-08`
- Snapshot anterior: `2026-06-02`
- Total anterior: **1583**
- Total atual: **1583**

## Resumo

- Entraram na base: **0**
- Saíram da base: **0**
- Registros com alguma alteração: **121**
- Registros com mudança de dados de origem: **115**
- Registros com mudança apenas em campos derivados/status: **6**

## Mudanças por status

- `status_suspensiva_calc`: **2**
- `status_pub_licitacao_calc`: **7**
- `status_homolog_licitacao_calc`: **0**
- `status_inicio_obra_calc`: **11**
- `status_regra_casa_civil_calc`: **0**
- `urgencia_suspensiva_calc`: **16**
- `fase_atual_calc`: **42**

## Leitura recomendada

- `categoria_alteracao = dados_e_derivados`: mudou dado de origem e isso também repercutiu em campos calculados.
- `categoria_alteracao = dados`: mudou apenas dado de origem, sem alterar status calculado.
- `categoria_alteracao = derivados_tempo`: o dado-base ficou igual; a alteração veio só de regra derivada/data corrente.
